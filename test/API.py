...
class TestAPIMethods(unittest.TestCase):
    ...
    def test_1(self):
        result = json.loads(BySykkelAPI.request("prefix"))
        self.assertTrue(len(result.get("stations")) > 0)

    def test_2(self):
        result1 = json.loads(BySykkelAPI.request("prefix"))
        result2 = json.loads(BySykkelAPI.request("prefix?bikes=0?docks=0"))
        self.assertEqual(len(result1.get("stations")), len(result2.get("stations")))

    def test_3(self):
        result1 = json.loads(BySykkelAPI.request("prefix"))
        result2 = json.loads(BySykkelAPI.request("prefix?bikes=20?docks=20"))
        self.assertTrue(len(result1.get("stations")) >= len(result2.get("stations")))

    def test_4(self):
        #TODO: Make test case not dependent on online data
        result = json.loads(BySykkelAPI.request("prefix?station_id=626"))
        self.assertEqual(result.get("station_id", 0), '626')

    def test_5(self):
        result = json.loads(BySykkelAPI.request("prefix?client_id=123"))
